...
@file: attention_wrapper.py
@time: 2019/7/16 16:58
    ...
    def __init__(self,
        ...
        with ops.name_scope(
            ...
            self._values = _prepare_memory(
                ...
                check_inner_dims_defined=check_inner_dims_defined)  # 确定参与计算的编码向量
            self._keys = (
                self.memory_layer(self._values) if self.memory_layer  # pylint: disable=not-callable
                else self._values)  # 权重与编码向量向乘进行预缓存
            self._batch_size = (
...
class BahdanauAttention(_BaseAttentionMechanism):
    ...
    def __init__(self,
        ...
        Args:
          ...
          score_mask_value: (optional): The mask value for score before passing into
            `probability_fn`. The default is -inf. Only used if
            `memory_sequence_length` is not None. 实数，不是向量，类似填充值
    ...
    def __call__(self, query, state):
        ...
        alignments = self._probability_fn(score, state)  # 获取attention weight
        next_state = alignments
        return alignments, next_state
...
def _compute_attention(attention_mechanism, cell_output, attention_state,
    ...
    alignments, next_attention_state = attention_mechanism(
        cell_output, state=attention_state)  # 计算权值向量， cell_output 为上一层的隐状态
    ...
    context = math_ops.matmul(expanded_alignments, attention_mechanism.values)  # 根据attention weight和编码向量计算glimpse

...
def _int64_feature_forward(value):
    """
    Wrapper for inserting int64 features into Example proto.
    :param value:
    :return:
...
def _int64_feature_backward(value):
    """
    Wrapper for inserting int64 features into Example proto.
    :param value:
    :return:
...
def _write_tfrecords(tfrecords_writer):
    ...
    while True:
        ...
        features = tf.train.Features(feature={
            'flabels': _int64_feature_forward(sample_label),
            'blabels': _int64_feature_backward(sample_label),
...
class _FeatureIO(object):
    ...
    def char_to_int(self, char):
        ...
        number, and then convert the ord number into int index which is stored
    ...
    def sparse_tensor_to_str(self, sparse_tensor, reverse=False):
        """
        :param sparse_tensor: prediction or ground truth label
        :param reverse: reverse the string
...
class FeatureReader(_FeatureIO):
    """
    ...
    def __init__(self, char_dict_path, ord_map_dict_path, flags='train'):
        ...
        :param flags:
        """
        super(FeatureReader, self).__init__(char_dict_path, ord_map_dict_path)
    ...
    @staticmethod
    def _augment_for_train(input_images, input_flabels, input_blabels, input_image_paths):
        """
        ...
        :return:
        """
        return input_images, input_flabels, input_blabels, input_image_paths

    @staticmethod
    def _augment_for_validation(input_images, input_flabels, input_blabels, input_image_paths):
        """
    ...
    @staticmethod
    def _normalize(input_images, input_flabels, input_blabels, input_image_paths):
        """
    ...
    def _extract_features_batch(serialized_batch):
        ...
        features = tf.parse_example(
            ...
            features={'images': tf.FixedLenFeature([], tf.string),
                      ...
                      'flabels': tf.VarLenFeature(tf.int64),
                      'blabels': tf.VarLenFeature(tf.int64),
                      }
        )
        ...
        images = tf.reshape(images, [bs, h, w, config.ENCODER.INPUT_CHANNELS])

        flabels = features['flabels']
        blabels = features['blabels']
        flabels = tf.cast(flabels, tf.int32)
        blabels = tf.cast(blabels, tf.int32)

        imagepaths = features['imagepaths']

        return images, flabels, blabels, imagepaths
...
class FeatureWriter(_FeatureIO):
    """
    ...
    def run(self):
        ...
        for i in range(self._writer_process_nums):
            ...
            print(self._tfrecords_save_dir)
            print(tfrecords_save_name)
